/**
 * @file dropdown.js
 * Dropdown functionality for usage in the flatify framework.
 *
...
(function( $ ){
    ...
    return this.each(function(){
      ...
      $(this).on( 'keydown', function( event ){
        ...
        if( event.keyCode == 9 ){
          ...
          if( $(this).next(opts.dropdown).find( 'li.active' ).length == 1 ){
            $( events ).trigger( 'selected', $.fn.search.retrieve( $(this).next(opts.dropdown).find( 'li.active' ).find( 'a' )) );
      ...
      $(this).on( 'focusin keyup', function( event ){
       ...
       if( event.keyCode == 13){
          ...
          $(events).trigger( 'selected', $.fn.search.retrieve($(selected).find( 'a' )));
          $(dropdown).hide().empty();
          ...
          thread = setTimeout(function () {
            ...
            $.ajax({
              ...
              // add hover to select
              $(dropdown).find( 'li' ).on( 'mouseenter', function(){
                $(dropdown).find('li.active').removeClass('active');
                selected = $(this).addClass('active');
              });

              // add click event
              $(dropdown).find( 'li a' ).on( 'mousedown', function(){
  ...
  $.fn.search.format = function( member ) {
    if( !member.infix )
      return "<a data-id=" + member.id + ">" + member.first_name + " " + member.last_name + "</a>";
    return "<a data-id=" + member.id + ">" + member.first_name + " " + member.infix + " " + member.last_name + "</a>";
  };
