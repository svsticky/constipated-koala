...
class CheckoutCard < ApplicationRecord
  ...
  def send_confirmation!
    ...
    return unless update(confirmation_token: token)

    Mailings::Checkout.confirmation_instructions(
      self,
      Rails.application.routes.url_helpers.confirmation_url(confirmation_token: token)
    ).deliver_now
  end
end
