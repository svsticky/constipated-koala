class CheckoutProduct < ApplicationRecord
  ...
  def content_type
    # TODO error message
    errors.add(:image, 'Wrong content_type') unless image.attached? && image.content_type.in?(['image/jpeg', 'image/png'])
  end
  ...
  def url
    return self.image.representation(resize: '128x128') if self.image.attached?
