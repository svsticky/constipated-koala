class MailchimpJob < ApplicationJob
  ...
  def perform(member, interests = Settings['mailchimp.interests'].values, create_on_missing = false, mailchimp_status = 'subscribed')
    ...
    request.merge!({ status_if_new: mailchimp_status }) if create_on_missing
    request.merge!({ interests: Settings['mailchimp.interests'].values.map{ |i| { i => interests.include?(i) }}.reduce(&:merge)})
