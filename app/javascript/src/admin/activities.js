...
$(document).on('change', '#activity_notes_input_type', function() {
  ...
  if (notesInputType === 'text') {
    var notesValue = $('#notes-options input').map(function() {
      return $(this).val();
    }).get().join('\n');
    notesContainer.html('<textarea class="form-control" id="activity_notes" name="activity[notes]">' + notesValue + '</textarea>');
  } else {
    var optionsHtml = '';
    ...
    if (optionsArray.length > 0) {
      ...
      optionsHtml += '<div class="col-md-12">' +
        '<div class="input-group mb-3">' +
        '<span class="input-group-text">Title</span>' +
        '<input class="form-control" type="text" name="activity[notes_options][]" value="' + titleValue + '">' +
        '</div>' +
        '</div>';
      ...
      optionsArray.slice(1).forEach(function(optionValue) {
        optionsHtml += '<div class="col-md-6">' +
          '<div class="input-group mb-3">' +
          '<input class="form-control" type="text" name="activity[notes_options][]" value="' + optionValue + '">' +
          '<div class="input-group-append">' +
          '<button class="btn btn-sm btn-danger delete-option" type="button">' +
          '<i class="fa fa-fw fa-minus"></i>' +
          '</button>' +
    ...
    notesContainer.html('<div class="row" id="notes-options">' + optionsHtml + '</div>' +
      '<div class="row">' +
      '<div class="col-md-12">' +
