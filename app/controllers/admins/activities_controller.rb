class Admins::ActivitiesController < ApplicationController
  ...
  def index    
    @activities = Activity.study_year( params['year'].blank? ? (Date.start_studyyear( Date.current().year ).year ) : params['year'].to_i).order(start_date: :desc)
    
    @years = (Activity.take(1).first.start_date.year .. Date.start_studyyear( Date.current().year ).year ).map{ |year| ["#{year}-#{year +1}", year] }.reverse
