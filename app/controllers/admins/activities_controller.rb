class Admins::ActivitiesController < ApplicationController
  ...
  def index    
    @activities = Activity.study_year(params['year'].to_i || Date.current().year ).order(start_date: :desc)
    @years = (Activity.take(1).first.start_date.year .. Date.current().year).map{ |year| ["#{year -1}-#{year}", year] }.reverse
